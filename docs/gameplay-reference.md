...
When Billy collides with a bottom-entry Gunman, that Gunman enters a measured
60-frame retreat state before its slot is released. Horse protection uses the
same retreat state; it is separate from the Horse's protection timer and does
not create a projectile or drop.
