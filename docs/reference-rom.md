...
Each Round initialization at `$E817-$E831` resets the event phase, actor timers,
global frame counter `$22`, and loop state, while `$E83D-$E845` reseeds the
random registers; `$E85E` restores Billy to NES `(128,188)` before loading its
event script. Boss arenas
...
the actor moves toward the road center from either side, follows the measured
full path through its final top-edge retreat and releases at frame 228. The
...
The late `at=4831,x=248,y=48,code=7,phase=1` branch matches all 582 controlled
state-machine samples across the map wrap and releases at `Y=253`.
The same-frame Boss-gate record `at=4863,x=248,y=80,code=7,phase=1` matches all
381 controlled state-machine samples through its left-edge release.
...
The same-frame `at=4511` entries are separated by allocation X: `x=152` matches
...
slot fractions and input windows: 1951 matches 1000 frames with attacks at
...
The `at=1631,x=184` bottom entry likewise reaches player-contact dispatch on
frame 58 with no successful shot, so it remains on the bounded contact path.
...
At `at=1535`, the bottom `x=120` entry reaches player-contact dispatch `0x41`
at frame 34; the same-time `x=184` entry matches 390 frames and releases through
the upper boundary. Neither produces a successful shot.
...
The top `at=1455,x=128,code=6,phase=0` entry matches 399 shared-state frames,
fires at frame 63, and releases through the upper screen boundary.
...
on its allocation frame instead of applying the ordinary `x+1` side step;
after that initializer correction all 776 no-shot state-machine frames match
through the NES `Y=255` release.
...
The naturally allocated `at=1727,x=248,y=96,code=7,phase=1` branch independently
...
releases at `Y=252`.
Round 1's `at=703,x=248,y=32,code=7,phase=1` event follows a separate
583-frame route, fires at frame 67, and releases at `Y=252`.
...
edge. The at655 code-8 phase-0 entry follows a separate 570-frame route to the
top edge.
The at1135 code-7 phase-0 entry follows a 307-frame route to the bottom edge.
The at1167 code-7 phase-0 entry follows a 299-frame route to the bottom edge.
...
The at1903 code-7 phase-0 entry follows a 732-frame route to the bottom edge.
...
Round 3's code 7 `y=64`, phase-1 entries use separate left/right traces of
581/384 frames; its code 8 left
...
captures the aim sector at that transition, then emits five dispatch `0x30`
shots 16 frames apart, beginning 16 frames later. The integer aim selects one
of three five-heading fans centered on sectors `12`, `16`, or `20`, captured
...
captures its aim when the attack state starts at frame 80, fires five dispatch
`0x30` shots at frames 96/112/128/144/160, then returns to
the edge and releases at frame 258. Runtime mirrors this path for the left-edge
records instead of applying code `14`'s top-entry path.
...
An isolated `at=1551,x=240` allocation (`fineX=176,fineY=0`) emits at 116
and releases after 207 frames. The following `at=1567,x=152` route
(`fineX=81,fineY=204`) emits at frame 116 and releases at frame 228; its
isolated `x=216,fineX=176,fineY=0` partner releases after 198 frames with the
same single emission.
...
slot route emits at frame 116 and releases after 200 frames. Event
...
Event `at=1887,x=96,phase=1` (`fineX=88,fineY=246`) throws at
...
The isolated `x=112,fineX=176,fineY=0` partner emits at 116 and releases after
224 frames.
...
`x=80,fineX=176,fineY=0` route emits at 116 and releases after 664 frames.
Event
...
116/153/190 and a separate neutral slot fraction.
Its isolated `x=128,fineX=44,fineY=0` partner emits at 116/153 and releases
after 332 frames.
...
The isolated `at=3055,x=88,phase=0` partner emits once at frame 116 and
...
holding those launch coordinates for the allocation frame. At frame 379 the
...
attack trace's multi-hop X/Y profile for 12,397 combat frames, then holds the
final sampled position in the ROM's post-route wait state; the attack random
gate and follow-up timing use the decoded ROM state.
...
records a later round with the same path. Round 1, Round 2, and Round 3 are
tracked; the other music loops remain generated until their ROM tracks are
captured.
...
and the special-ammo decrement at `$EECF`. Runtime applies the same
all-or-nothing rule.
