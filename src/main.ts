...
class GunSmokeGame {
  ...
  private updateUnit(unit: Unit, delta: number): void {
    ...
    if (unit.kind === "enemyBullet" && unit.projectileType === "rock") {
      if (unit.exploding) {
        unit.targetY = (unit.targetY ?? unit.y) + ROM_OBJECT_DROP_SPEED * delta;
        unit.y = unit.targetY;
