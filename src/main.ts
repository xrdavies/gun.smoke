...
import { RIFLEMAN_ATTACK_STATE_FRAME, RIFLEMAN_LIFETIME } from "./game-constants";
...
class GunSmokeGame {
  ...
  private updateUnit(unit: Unit, delta: number): void {
    ...
    if (unit.kind === "enemy") {
      const followsRomScroll = unit.romBehavior !== undefined && !(unit.enemyType === "backstabber" && (unit.romBehavior === 3 || unit.romBehavior === 8));
