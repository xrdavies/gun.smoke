...
class GunSmokeGame {
  ...
  private spawnBoss(): void {
    ...
    const boss = this.spawnUnit(
      "boss",
      isBanditBill ? BANDIT_BILL_ENTRY_X : isCutter ? CUTTER_ENTRY_X : 480,
      this.scroll + (isBanditBill ? BANDIT_BILL_ENTRY_Y_LANES[Math.floor(this.nextRandom() * BANDIT_BILL_ENTRY_Y_LANES.length)] ?? 360 : isCutter ? CUTTER_ENTRY_Y_LANES[Math.floor(this.nextRandom() * CUTTER_ENTRY_Y_LANES.length)] ?? 378 : 90),
      definition.bossHp * 4,
    );
    if (isBanditBill || isCutter) {
      boss.bossEntryY = boss.y - this.scroll;
      boss.vx = isBanditBill ? BANDIT_BILL_ENTRY_SPEED_X : CUTTER_ENTRY_SPEED_X;
    }
  ...
  private updateUnit(unit: Unit, delta: number): void {
    ...
    if (unit.kind === "enemy") {
      ...
      unit.x += unit.vx * delta;
      const edgeEntryBoss = this.stage === 1 || this.stage === 2;
