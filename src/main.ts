...
class GunSmokeGame {
  ...
  private spawnBoss(): void {
    ...
    const boss = this.spawnUnit(
      "boss",
      this.stage === 1 ? BANDIT_BILL_ENTRY_X : 480,
      this.scroll + (this.stage === 1 ? BANDIT_BILL_ENTRY_Y : 90),
      definition.bossHp * 4,
    );
    if (this.stage === 1) boss.vx = BANDIT_BILL_ENTRY_SPEED_X;
  ...
  private updateUnit(unit: Unit, delta: number): void {
    ...
    if (unit.kind === "enemy") {
      ...
      unit.x += unit.vx * delta;
      const minBossX = this.stage === 1 ? 0 : 380;
      const maxBossX = this.stage === 1 ? 960 : 580;
      if (unit.x < minBossX || unit.x > maxBossX) unit.vx *= -1;
      if (this.stage === 1) unit.y = this.scroll + (unit.age < unit.invulnerableUntil ? 430 : BANDIT_BILL_ENTRY_Y);
