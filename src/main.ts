...
class GunSmokeGame {
  ...
  private spawnBoss(): void {
    ...
    const boss = this.spawnUnit(
      "boss",
      isBanditBill ? BANDIT_BILL_ENTRY_X : isCutter ? CUTTER_ENTRY_X : isDevilHawk ? DEVIL_HAWK_ENTRY_X : 480,
      this.scroll + (isBanditBill ? BANDIT_BILL_ENTRY_Y_LANES[Math.floor(this.nextRandom() * BANDIT_BILL_ENTRY_Y_LANES.length)] ?? 360 : isCutter ? CUTTER_ENTRY_Y_LANES[Math.floor(this.nextRandom() * CUTTER_ENTRY_Y_LANES.length)] ?? 378 : isDevilHawk ? DEVIL_HAWK_ENTRY_Y_LANES[Math.floor(this.nextRandom() * DEVIL_HAWK_ENTRY_Y_LANES.length)] ?? 378 : 90),
      definition.bossHp * 4,
    );
    if (isBanditBill || isCutter || isDevilHawk) {
