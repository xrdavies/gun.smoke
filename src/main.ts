...
interface Unit {
  ...
  gunmanBottomContactState?: GunmanBottomContactState;
  gunmanBottomContactStartFrame?: number;
...
class GunSmokeGame {
  ...
  private spawnRomEnemyEvent(event: RomEnemyEvent): void {
    ...
    if (flankCode !== undefined && gunmanFlankUsesDynamicState(flankCode, event.y, this.stage, event.phase, event.at, event.x > 128)) {
      ...
      if (this.stage === 6 && flankCode === 8 && event.at === 2143) enemy.gunmanFlankState.x -= 1;
    }
  ...
  private fireBoss(boss: Unit, effectiveAge = boss.age, devilFullFan?: boolean): void {
    ...
    if (this.stage === 4) {
      const prepareFrames = boss.bossCycleStart === undefined ? NINJA_BOSS_INITIAL_PREPARE_FRAMES : NINJA_BOSS_REENTRY_PREPARE_FRAMES;
      const opportunityIndex = boss.shotOpportunityIndex ?? 0;
      ...
      boss.volleysFired += 1;
      boss.fired = true;
      boss.shotOpportunityIndex = opportunityIndex + 1;
      const nextFrame = prepareFrames[opportunityIndex + 1];
      boss.nextFireAt = nextFrame === undefined ? Number.POSITIVE_INFINITY : (boss.bossCycleStart ?? 0) + nextFrame / NES_FRAME_RATE;
      return;
    }
  }
  ...
  private updateUnit(unit: Unit, delta: number, scrollDelta = 0): void {
    ...
    if (unit.kind === "enemy") {
      if (unit.gunmanBottomContactState && unit.gunmanBottomContactStartFrame !== undefined) {
        const frame = Math.max(0, Math.floor(unit.age * NES_FRAME_RATE) - unit.gunmanBottomContactStartFrame);
        advanceGunmanBottomContact(unit.gunmanBottomContactState, frame);
  ...
  private isBossVulnerable(unit: Unit): boolean {
    ...
    return bossIsVulnerable(this.stage, unit.age, unit.invulnerableUntil, unit.fired);
  }
