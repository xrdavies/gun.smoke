...
class GunSmokeGame {
  ...
  private updateUnit(unit: Unit, delta: number): void {
    ...
    if (unit.kind === "enemy") {
      ...
      if (unit.enemyType === "sniper" && (unit.y - this.scroll) / NES_WORLD_Y_SCALE >= ROM_ENEMY_RELEASE_Y_NES) unit.hp = 0;
      ...
      unit.y += ROM_OBJECT_DROP_SPEED * delta;
  ...
  private defeatTarget(target: Unit): void {
    ...
    if (target.kind === "barrel") {
      ...
      target.maxAge = target.age + BOSS_DEFEAT_ANIMATION_DURATION;
