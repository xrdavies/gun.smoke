...
import { RIFLEMAN_ATTACK_STATE_FRAME, RIFLEMAN_LIFETIME, riflemanCanAttack, riflemanPosition, RIFLEMAN_SIDE_LIFETIME, RIFLEMAN_SIDE_SHOT_FRAMES, riflemanSidePosition, sniperProjectileVelocity } from "./game-constants";
...
class GunSmokeGame {
  ...
  private isPlayerBlocked(x: number, y: number): boolean {
    return roundCollisionBlocks(this.stage, this.scroll, x, y) || (ROUND_OBSTACLES[this.stage - 1] ?? []).some((obstacle) => obstacleBlocks(obstacle, x, y));
  }
  ...
  private spawnBoss(): void {
    ...
    const ninjaBossLane = isNinjaBoss ? NINJA_BOSS_ENTRY_LANES[Math.floor(this.nextRandom() * NINJA_BOSS_ENTRY_LANES.length)] : undefined;
    ...
    if (unit.kind === "enemy") {
      ...
      if (unit.enemyType === "backstabber") {
        ...
        if (tracedNinja) {
          ...
          if (unit.age < NINJA_FIRST_SHOT_DELAY) {
            unit.x = originX;
        ...
        if (shouldFire) {
          ...
          for (const spread of spreads) {
            const projectile = this.spawnEnemyProjectile(unit.x + (tracedFirebreather ? FIREBREATHER_PROJECTILE_OFFSET_NES[0] * NES_WORLD_X_SCALE : 0), unit.y + (tracedFirebreather ? FIREBREATHER_PROJECTILE_OFFSET_NES[1] * NES_WORLD_Y_SCALE : 12));
            if (!projectile) break;
            projectile.projectileType = "fireball";
        ...
        if (bottomGunmanFromLeft === undefined && flankGunman === undefined && tracedGunman && unit.age <= GUNMAN_ENTRY_PATH_NES.at(-1)![0] / NES_FRAME_RATE) unit.x = unit.romOriginX ?? unit.x;
        if (bottomGunmanFromLeft === undefined && flankGunman === undefined && (!tracedGunman || unit.age > GUNMAN_ENTRY_PATH_NES[1]![0] / NES_FRAME_RATE)) unit.x += Math.sin(unit.age * 3 + unit.phase) * 18 * delta;
        const shotFrames = bottomGunmanRoute !== undefined ? GUNMAN_BOTTOM_SHOT_FRAMES[bottomGunmanRoute] : flankGunman !== undefined ? GUNMAN_FLANK_SHOT_FRAMES[flankGunman] : [39];
      ...
      unit.sprite.visible = !ninjaTeleporting && unit.age >= unit.invulnerableUntil;
  ...
  private resolveCollisions(): void {
    ...
    for (const unit of this.units.filter((candidate) => candidate.hp > 0)) {
      ...
      if (unit.kind === "moneyBag" || unit.kind === "item" || unit.kind === "ammo") {
        ...
        this.takeHit();
        if (unit.kind !== "boss" && !(unit.kind === "enemyBullet" && unit.projectileType === "dynamite" && !dynamiteContactIsDefusable(unit.age))) unit.hp = 0;
      }
    }
  }
