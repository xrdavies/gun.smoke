...
import { bossDefeatAnimationDuration, REGULAR_BOSS_STAGE_CLEAR_DELAY, WINGATE_ENDING_INPUT_DELAY, WINGATE_FINAL_DEFEAT_ANIMATION_DURATION, WINGATE_FINAL_ENDING_DELAY } from "./game-constants";
...
class GunSmokeGame {
  ...
  private updateUnit(unit: Unit, delta: number, scrollDelta = 0): void {
    ...
    if (unit.kind === "enemy") {
      ...
      if (unit.enemyType === "backstabber") {
        ...
        if (!tracedGunman && flankGunman === undefined) unit.x += Math.sin(unit.age * 3 + unit.phase) * 18 * delta;
        const topGunman = !bottomGunman && flankGunman === undefined;
        const explicitShotFrames = flankGunman !== undefined || dynamicTopGunman || dynamicBottomGunman ? gunmanFlankEventShotFrames(this.stage, unit.romEventAt, unit.romOriginX === undefined ? undefined : unit.romOriginX / NES_WORLD_X_SCALE) : undefined;
        ...
        if (unit.hp > 0 && unit.romBehavior === 2 && (timedOpportunity || explicitOpportunity)) {
          const facingHeading = dynamicBottomGunman
            ...
            : nesAimHeading(previousX, previousY, unit.x, unit.y);
          const aimHeading = nesAimHeading(unit.x, unit.y, this.player.x, this.player.y);
          if (gunmanCanFire(facingHeading, aimHeading)) {
            const projectile = this.spawnEnemyProjectile(unit.x, unit.y);
            if (projectile) {
              unit.fired = true;
              unit.volleysFired += 1;
  ...
  private defeatTarget(target: Unit): void {
    ...
    if (target.kind === "barrel") {
      ...
      this.bossSpawned = false;
      this.stageClearClock = finalWingate ? WINGATE_FINAL_ENDING_DELAY : REGULAR_BOSS_STAGE_CLEAR_DELAY;
