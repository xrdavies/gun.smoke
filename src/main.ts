...
import { ENEMY_REWARDS } from "./game-constants";
...
class GunSmokeGame {
  ...
  private spawnUnit(kind: UnitKind, x: number, y: number, hp: number, enemyType?: EnemyType, itemType?: ItemType): Unit {
    ...
    const unit: Unit = {
      ...
      value: isBoss ? bossReward(this.stage, this.wingatePhase) : kind === "enemy" && enemyType ? ENEMY_REWARDS[enemyType] : kind === "moneyBag" ? 200 : kind === "ammo" || kind === "item" || kind === "barrel" ? 0 : 100,
