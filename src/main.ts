  ...
  private updateUnit(unit: Unit, delta: number): void {
    ...
    if (unit.kind === "enemy") {
      ...
      if (unit.enemyType === "backstabber") {
        ...
        const shotFrames = bottomGunmanRoute !== undefined ? GUNMAN_BOTTOM_SHOT_FRAMES[bottomGunmanRoute] : flankGunman !== undefined ? GUNMAN_FLANK_SHOT_FRAMES[flankGunman] : GUNMAN_TOP_SHOT_FRAMES;
        ...
        if (unit.romBehavior === 2 && nextShotFrame !== undefined && unit.age >= nextShotFrame / NES_FRAME_RATE) {
          unit.fired = true;
          unit.volleysFired += 1;
          const facingHeading = nesAimHeading(previousX, previousY, unit.x, unit.y);
          const aimHeading = nesAimHeading(unit.x, unit.y, this.player.x, this.player.y);
          if (bottomGunmanRoute !== undefined || flankGunman !== undefined || gunmanCanFire(facingHeading, aimHeading)) {
            const projectile = this.spawnEnemyProjectile(unit.x, unit.y);
            if (projectile) {
