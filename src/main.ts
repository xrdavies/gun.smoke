...
import { advanceGunmanFlankMovement, advanceSniperFiring, createGunmanBottomMovementState, createGunmanFlankMovementState, createGunmanTopMovementState, createSniperFiringState, GUNMAN_BOTTOM_BRANCH_FRAME, GUNMAN_BOTTOM_DYNAMIC_HANDOFF_FRAME, GUNMAN_BOTTOM_LIFETIMES, gunmanBottomDynamicPosition, gunmanBottomFirstOpportunityFrame, gunmanBottomPosition, gunmanBottomRoute, GUNMAN_BOTTOM_SHOT_FRAMES, gunmanBottomUsesDynamicState, gunmanCanFire, GUNMAN_FLANK_INITIAL_STATE_FRAMES, gunmanFlankEventShotFrames, gunmanFlankFirstOpportunityFrame, gunmanFlankLifetime, gunmanFlankMovementFacingHeading, gunmanFlankUsesDynamicState, GUNMAN_LIFETIME, GUNMAN_TOP_LIFETIMES_FRAMES, gunmanFirstOpportunityFrame, gunmanFlankPosition, gunmanTopBranch, gunmanTopHeading, gunmanTopPosition, gunmanTopUsesDynamicState, gunmanProjectileVelocity, GUNMAN_SHOT_OPPORTUNITY_INTERVAL, mediumProjectileHeadingVelocity, mediumProjectileVelocity, type GunmanFlankMovementState, type SniperFiringState } from "./game-constants";
...
import { bossDefeatAnimationDuration, WINGATE_ENDING_INPUT_DELAY, WINGATE_FINAL_DEFEAT_ANIMATION_DURATION, WINGATE_FINAL_ENDING_DELAY } from "./game-constants";
import { CUTTER_ATTACK_INTERVAL, CUTTER_BOOMERANG_FIRST_TURN_DELAY, CUTTER_BOOMERANG_FOLLOWUP_DELAY, CUTTER_BOOMERANG_HEADINGS, cutterBoomerangHeadingToward, CUTTER_BOOMERANG_OUTWARD_TARGETS_NES, CUTTER_BOOMERANG_REAIM_Y_NES, CUTTER_BOOMERANG_SPAWN_NES, CUTTER_BOOMERANG_TURN_INTERVAL, cutterBoomerangTurn, cutterBoomerangVelocity, CUTTER_FIRST_ATTACK_DELAY } from "./game-constants";
...
class GunSmokeGame {
  ...
  private update(delta: number): void {
    ...
    if (this.wingateRespawnClock > 0) {
      ...
      if (this.wingateRespawnClock <= 0) {
        ...
        boss.invulnerableUntil = WINGATE_ENTRY_INVULNERABILITY;
  ...
  private spawnRomObjectEvent(event: RomObjectEvent): void {
    ...
    if (event.semantic === "weaponShop" || event.semantic === "supplyShop") {
      ...
      keeper.romOriginY = romObjectWorldY(event);
      return;
    }
    if (event.semantic === "sceneObject" && ROM_SCENE_PROP_DISPATCH_TYPES.includes(event.dispatchType as 8)) {
      ...
      prop.romOriginY = romObjectWorldY(event);
    ...
    container.romOriginY = romObjectWorldY(event);
  }
  ...
  private spawnRomEnemyEvent(event: RomEnemyEvent): void {
    ...
    if (ROM_FALLING_ROCK_BEHAVIORS.includes(event.behavior as 5)) {
      ...
      rock.romSpawnFineX = (this.romEnemyFineX[romSlot ?? 0] ?? 0) / 256;
      rock.romSpawnFineY = (this.romEnemyFineY[romSlot ?? 0] ?? 0) / 256;
      rock.romFineX = rock.romSpawnFineX;
      rock.romFineY = rock.romSpawnFineY;
      rock.x += rock.romSpawnFineX * NES_WORLD_X_SCALE;
      rock.y += rock.romSpawnFineY * NES_WORLD_Y_SCALE;
    ...
    enemy.romSpawnFineX = (this.romEnemyFineX[romSlot ?? 0] ?? 0) / 256;
    enemy.romSpawnFineY = (this.romEnemyFineY[romSlot ?? 0] ?? 0) / 256;
    enemy.romFineX = enemy.romSpawnFineX;
    enemy.romFineY = enemy.romSpawnFineY;
    enemy.x += enemy.romSpawnFineX * NES_WORLD_X_SCALE;
    enemy.y += enemy.romSpawnFineY * NES_WORLD_Y_SCALE;
    if (event.behavior === 0 && (event.entityCode === 1 || event.entityCode === 2)) enemy.sniperFiringState = createSniperFiringState(event.entityCode, romRandomSeed ?? 0);
    if (event.behavior === 3) {
    ...
    if (event.behavior === 2 && event.entityCode === 6 && gunmanTopUsesDynamicState(this.stage, event.at)) enemy.maxAge = Number.POSITIVE_INFINITY;
    if (event.behavior === 6) {
    ...
    if (event.behavior === 9) enemy.maxAge = Number.POSITIVE_INFINITY;
  ...
  private fireBoss(boss: Unit, effectiveAge = boss.age, devilFullFan?: boolean): void {
    ...
    if (this.stage === 2) {
      for (let index = 0; index < CUTTER_BOOMERANG_SPAWN_NES.length; index += 1) {
        ...
        projectile.maxAge = Number.POSITIVE_INFINITY;
        projectile.boomerangHeading = heading;
        projectile.radius = 7;
      }
      boss.volleysFired += 1;
      boss.fired = true;
      if (boss.cutterFollowup) {
        boss.cutterFollowup = false;
        this.bossFireClock = CUTTER_ATTACK_INTERVAL;
      } else {
        boss.cutterFollowup = true;
        this.bossFireClock = CUTTER_BOOMERANG_FOLLOWUP_DELAY;
      }
      this.beep(186, 0.045);
      return;
    }
  ...
  private spawnBoss(): void {
    ...
    if (isBanditBill || isCutter || isDevilHawk || isNinjaBoss || isFatmanJoe || isFirstWingate) {
      ...
      if (isFirstWingate) boss.invulnerableUntil = WINGATE_ENTRY_INVULNERABILITY;
  ...
  private spawnUnit(kind: UnitKind, x: number, y: number, hp: number, enemyType?: EnemyType, itemType?: ItemType): Unit {
    ...
    const unit: Unit = {
      kind, enemyType, itemType, projectileType: kind === "enemyBullet" ? "bullet" : undefined, sprite, x, y, animation, defeatAnimationDuration: undefined, shopIndex: undefined, romBehavior: undefined, romEntityCode: undefined, romEventAt: undefined, romRandomSeed: undefined, romPhase: undefined, romFlags: undefined, romPool: undefined, romOriginX: undefined, romOriginY: undefined, romFineX: undefined, romFineY: undefined, romSpawnFineX: undefined, romSpawnFineY: undefined, targetX: undefined, targetY: undefined, gunmanBottomRoute: undefined, gunmanTopBranch: undefined, gunmanFlankState: undefined, sniperFiringState: undefined, backstabberRaidState: undefined, romSlot: undefined, riflemanAimHeading: undefined, riflemanAttackStarted: undefined, hatchetState: undefined, firebreatherState: undefined, spearState: undefined, bomberState: undefined, bomberDirection: undefined, banditState: undefined, cutterState: undefined, cutterFollowup: undefined, boomerangHeading: undefined, bossCycleStart: undefined, bossNextTeleportAt: undefined, rockNextBoundary: undefined, rockPhase: undefined,
    ...
    if (unit.kind === "enemyBullet" && unit.projectileType === "rock") {
      if (unit.exploding) {
        ...
        if (unit.age * NES_FRAME_RATE >= boundary) {
          ...
          const screenX = (unit.romOriginX ?? unit.x) / NES_WORLD_X_SCALE + (unit.romSpawnFineX ?? 0) + previousOffsetX + probeX;
          const screenY = (unit.romOriginY ?? unit.y - this.scroll) / NES_WORLD_Y_SCALE + (unit.romSpawnFineY ?? 0) + previousOffsetY + probeY;
          if (boundary >= ROCK_IMPACT_DELAY * NES_FRAME_RATE || !roundActorCollisionAtNes(this.stage, this.scroll, screenX, screenY)) {
            unit.exploding = true;
            unit.targetY = unit.y;
    ...
    if (unit.kind === "enemy") {
      ...
      if (unit.enemyType === "backstabber") {
        if (unit.romBehavior === 3 && unit.backstabberRaidState) {
          ...
          unit.y = this.scroll + backstabberAmbushY(unit.age, unit.romSpawnFineY);
        } else {
        ...
        if (tracedNinja) {
          if (unit.ninjaState) {
            const result = advanceNinja(
              ...
              (probeX, probeY) => roundActorCollisionAtNes(this.stage, this.scroll, probeX, probeY),
        ...
        if (sideRifleman) {
          ...
          unit.x = (unit.romOriginX ?? unit.x) + (unit.romSpawnFineX ?? 0) * NES_WORLD_X_SCALE;
          unit.y = this.scroll + (unit.romOriginY ?? 0) + (y + (unit.romSpawnFineY ?? 0)) * NES_WORLD_Y_SCALE;
        } else {
        ...
        const frame = Math.max(1, Math.round(unit.age * NES_FRAME_RATE));
        const aimHeading = nesAimHeading(unit.x, unit.y, this.player.x, this.player.y);
        const shouldFire = unit.romBehavior === 0 && unit.sniperFiringState
          ? advanceSniperFiring(unit.sniperFiringState, frame, (unit.y - this.scroll) / NES_WORLD_Y_SCALE, aimHeading)
        ...
        if (tracedBomber && unit.bomberState === "entry") {
          if (unit.age <= BOMBER_ENTRY_DURATION) {
            unit.x = (unit.romOriginX ?? unit.x) + (unit.romSpawnFineX ?? 0) * NES_WORLD_X_SCALE;
            unit.y = this.scroll + (unit.romOriginY ?? 0) + bomberOpeningY(unit.age) + (unit.romSpawnFineY ?? 0) * NES_WORLD_Y_SCALE;
          } else {
        ...
        const bottomGunmanFromLeft = bottomGunman && !dynamicBottomGunman ? (unit.romOriginX ?? unit.x) <= (unit.targetX ?? this.player.x) : undefined;
        const bottomGunmanRoute = bottomGunman && !dynamicBottomGunman ? unit.gunmanBottomRoute ?? "near" : undefined;
        ...
        const tracedTopGunman = tracedGunman && !bottomGunman && !dynamicTopGunman && flankGunman === undefined;
        ...
        if (dynamicBottomGunman) {
          const frame = Math.round(unit.age * NES_FRAME_RATE);
          if (frame < GUNMAN_BOTTOM_DYNAMIC_HANDOFF_FRAME) {
            [unit.x, unit.y] = gunmanBottomDynamicPosition(unit.age, (unit.romOriginX ?? unit.x) / NES_WORLD_X_SCALE, (unit.romSpawnFineX ?? 0) * 256, (unit.romSpawnFineY ?? 0) * 256);
            unit.y += this.scroll;
          } else {
            unit.gunmanFlankState ??= createGunmanBottomMovementState((unit.romOriginX ?? unit.x) / NES_WORLD_X_SCALE, (unit.romSpawnFineX ?? 0) * 256, (unit.romSpawnFineY ?? 0) * 256);
            advanceGunmanFlankMovement(unit.gunmanFlankState, frame, this.player.x / NES_WORLD_X_SCALE, (this.player.y - this.scroll) / NES_WORLD_Y_SCALE, (probeX, probeY) => roundActorCollisionAtNes(this.stage, this.scroll, probeX, probeY));
        ...
        } else if (bottomGunmanFromLeft !== undefined && bottomGunmanRoute !== undefined) {
          const [offsetX, offsetY] = gunmanBottomPosition(bottomGunmanRoute, bottomGunmanFromLeft, unit.age);
          unit.x = (unit.romOriginX ?? unit.x) + (offsetX + (unit.romSpawnFineX ?? 0)) * NES_WORLD_X_SCALE;
          unit.y = this.scroll + (unit.romOriginY ?? 0) + (offsetY + (unit.romSpawnFineY ?? 0)) * NES_WORLD_Y_SCALE;
        } else if (dynamicTopGunman && unit.gunmanFlankState) {
          advanceGunmanFlankMovement(unit.gunmanFlankState, Math.round(unit.age * NES_FRAME_RATE), this.player.x / NES_WORLD_X_SCALE, (this.player.y - this.scroll) / NES_WORLD_Y_SCALE, (probeX, probeY) => roundActorCollisionAtNes(this.stage, this.scroll, probeX, probeY));
          unit.x = unit.gunmanFlankState.x * NES_WORLD_X_SCALE;
          unit.y = this.scroll + unit.gunmanFlankState.y * NES_WORLD_Y_SCALE;
          if (unit.gunmanFlankState.dead) unit.hp = 0;
          ...
          if (unit.gunmanFlankState) {
            ...
            unit.y = this.scroll + (unit.romOriginY ?? 0) + offsetY * NES_WORLD_Y_SCALE + (unit.romSpawnFineY ?? 0) * NES_WORLD_Y_SCALE;
          }
          ...
          unit.x = x + (unit.romSpawnFineX ?? 0) * NES_WORLD_X_SCALE;
          unit.y = this.scroll + y + (unit.romSpawnFineY ?? 0) * NES_WORLD_Y_SCALE;
        } else {
          unit.x += unit.vx * delta;
          unit.y += unit.vy * delta;
        }
        ...
        const explicitShotFrames = bottomGunmanRoute !== undefined
          ...
          : flankGunman !== undefined || dynamicTopGunman || dynamicBottomGunman ? gunmanFlankEventShotFrames(this.stage, unit.romEventAt, unit.romOriginX === undefined ? undefined : unit.romOriginX / NES_WORLD_X_SCALE) : undefined;
        ...
        if (timedGunman && unit.nextFireAt === 0) unit.nextFireAt = (flankGunman === undefined
          ? dynamicBottomGunman
            ? gunmanBottomFirstOpportunityFrame(unit.romRandomSeed ?? 0, this.stage, unit.romEventAt)
            : gunmanFirstOpportunityFrame(unit.romRandomSeed ?? 0, (unit.romOriginY ?? 0) / NES_WORLD_Y_SCALE)
          : gunmanFlankFirstOpportunityFrame(unit.romRandomSeed ?? 0, (unit.romOriginY ?? 0) / NES_WORLD_Y_SCALE, this.stage, flankGunman, unit.romPhase ?? 0, unit.romEventAt)) / NES_FRAME_RATE;
        const nextShotFrame = explicitShotFrames?.[unit.shotOpportunityIndex ?? 0];
        const timedOpportunity = timedGunman && unit.age >= unit.nextFireAt;
        const explicitOpportunity = nextShotFrame !== undefined && unit.age >= nextShotFrame / NES_FRAME_RATE;
        if (unit.hp > 0 && unit.romBehavior === 2 && (timedOpportunity || explicitOpportunity)) {
          const facingHeading = dynamicBottomGunman
            ? unit.gunmanFlankState ? gunmanFlankMovementFacingHeading(unit.gunmanFlankState) : 0
            : tracedTopGunman
          ...
          if (explicitOpportunity) unit.shotOpportunityIndex = (unit.shotOpportunityIndex ?? 0) + 1;
        }
      }
      ...
      if (!unit.exploding && unit.romOriginY !== undefined) unit.y = this.scroll + romObjectScreenY(unit.age, unit.romOriginY);
      else if (!unit.exploding) unit.y += scrollDelta * 2;
      ...
      if (unit.romOriginY !== undefined) unit.y = this.scroll + romPickupScreenY(unit.age, unit.romOriginY);
      else unit.y += scrollDelta * 2;
  ...
  private startNinjaBossTeleport(unit: Unit, triggerAge = unit.age): void {
    ...
    unit.sprite.visible = false;
    this.bossFireClock = NINJA_BOSS_TELEPORT_DELAY + NINJA_BOSS_REENTRY_PREPARE_DELAY;
...
if (import.meta.env.DEV) Object.defineProperty(window, "__setGunSmokeRound", { value: (stage: number) => {
  ...
  game.fireClock = 0;
  game.fireMask = 0;
  game.bombLatch = false;
  game.invulnerable = 0;
  game.invulnerableDestroysEnemies = false;
  game.deathClock = 0;
  game.deathCommitted = false;
  game.stageClearClock = 0;
  game.smartBombArmed = false;
  game.shopOpen = false;
  game.inventoryOpen = false;
  game.bossFireClock = 1;
  game.hasHorse = false;
  game.horseHealth = 0;
  game.horseSprite.visible = false;
