...
import { WINGATE_ATTACK_INTERVAL, WINGATE_BULLET_LIFETIME, wingateCanFire, WINGATE_ENTRY_RUSH_DURATION, WINGATE_ENTRY_RUSH_SPEED, WINGATE_FIRST_SHOT_DELAY, WINGATE_MOVEMENT_SPEED, WINGATE_PROJECTILE_X_OFFSET_NES, WINGATE_PROJECTILE_Y_OFFSET_NES, wingateProjectileVelocity, WINGATE_SECOND_FIRST_SHOT_DELAY, wingateCombatY, wingateRushOffset } from "./game-constants";
...
class GunSmokeGame {
  ...
  private spawnRomEnemyEvent(event: RomEnemyEvent): void {
    ...
    if (event.behavior === 6) enemy.maxAge = NINJA_LIFETIME;
  ...
  private updateUnit(unit: Unit, delta: number): void {
    ...
    if (unit.kind === "enemy") {
      ...
      if (unit.enemyType === "backstabber") {
        ...
        if (shouldFire) {
          ...
          for (const spread of spreads) {
            ...
            if (!projectile) break;
            projectile.projectileType = "fireball";
            if (tracedFirebreather) [projectile.vx, projectile.vy] = mediumProjectileVelocity(unit.x, unit.y, this.player.x, this.player.y, sideFirebreather);
            else {
        ...
        const shotFrames = bottomGunmanRoute !== undefined ? GUNMAN_BOTTOM_SHOT_FRAMES[bottomGunmanRoute] : flankGunman !== undefined ? GUNMAN_FLANK_SHOT_FRAMES[flankGunman] : undefined;
        const nextShotFrame = shotFrames?.[unit.volleysFired];
        const topOpportunity = bottomGunmanRoute === undefined && flankGunman === undefined && !unit.fired && unit.age >= GUNMAN_FIRST_SHOT_DELAY && unit.age * NES_FRAME_RATE < GUNMAN_TOP_SHOT_END_FRAME;
        if (unit.romBehavior === 2 && (topOpportunity || nextShotFrame !== undefined && unit.age >= nextShotFrame / NES_FRAME_RATE)) {
      ...
      if (this.stage === 1 && unit.age <= BANDIT_BILL_ENTRY_DURATION) unit.x = unit.bossEntryX ?? unit.x;
      ...
      else if (this.stage === 6 && unit.bossEntryY !== undefined && unit.age <= WINGATE_ENTRY_DURATION + WINGATE_ENTRY_RUSH_DURATION) {
        unit.x = (unit.bossEntryX ?? unit.x) + wingateRushOffset((unit.age - WINGATE_ENTRY_DURATION) * NES_FRAME_RATE) * NES_WORLD_X_SCALE;
      }
      else {
        ...
        if (this.stage === 6 && unit.bossEntryY !== undefined) {
          ...
          unit.vx = direction * WINGATE_MOVEMENT_SPEED;
        }
