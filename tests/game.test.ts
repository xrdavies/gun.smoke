...
describe("Gun.Smoke vertical slice", () => {
  ...
  it("carries only Magnum damage left after a piercing hit", () => {
    expect([piercingDamageAfterHit(3, 1), piercingDamageAfterHit(3, 3), piercingDamageAfterHit(3, 4)]).toEqual([
      { damage: 2, consumed: false },
      { damage: 0, consumed: true },
      { damage: 0, consumed: true },
    ]);
  });
  ...
  it("matches the traced Spear timing", () => {
    ...
    const replay = (sideEntry: boolean) => {
      ...
      for (let frame = 1; frame <= 304; frame += 1) {
        const result = advanceSpear(state, frame, 136, 188, () => 2);
        for (const heading of result.shots) shots.push({ frame, heading });
  ...
  it("matches the traced first Wingate entrance", () => {
    ...
    expect(wingateCombatX(WINGATE_ENTRY_DURATION + 34 / NES_FRAME_RATE, 0, 152) * NES_WORLD_X_SCALE).toBe(472.5);
    ...
    expect(wingateCombatX(WINGATE_ENTRY_DURATION + 3417 / NES_FRAME_RATE, 0, 152)).toBeCloseTo(134, 9);
    ...
    expect(wingateRushOffset(34, 104)).toBe(26);
