...
describe("Gun.Smoke vertical slice", () => {
  ...
  it("continues Bandit Bill from the sampled route", () => {
    ...
    expect([BANDIT_BILL_ATTACK_PAUSE_FRAMES, BANDIT_BILL_ROUTE_HANDOFF_PAUSE_FRAMES]).toEqual([37, 24]);
  ...
  it("matches the traced first Wingate entrance", () => {
    ...
    expect([Math.floor(movement.x), Math.floor(movement.y)]).toEqual([126, 56]);
    ...
    expect({ mode: movement.mode, heading: movement.heading, segment: movement.segmentFrames, gait: movement.gait }).toEqual({ mode: "correction", heading: 0x5c, segment: 7, gait: 4 });
    expect([Math.floor(movement.x), Math.floor(movement.y)]).toEqual([115, 99]);
    advanceWingateMovement(movement, 460, () => 0);
    expect(movement.mode).toBe("move");
    expect([Math.floor(movement.x), Math.floor(movement.y)]).toEqual([123, 54]);
    const realMovement = createWingateMovementState(192, 1);
    advanceWingateMovement(realMovement, 186, () => 0);
    expect([Math.floor(realMovement.x), Math.floor(realMovement.y)]).toEqual([165, 50]);
