...
describe("Gun.Smoke vertical slice", () => {
  ...
  it("keeps collision helpers bounded and Euclidean", () => {
    ...
    expect({ enemies: ENEMY_COLLISION_HALF_SIZE_NES, containers: CONTAINER_COLLISION_HALF_SIZE_NES, projectiles: ENEMY_PROJECTILE_COLLISION_HALF_SIZE_NES }).toEqual({ enemies: [9, 8], containers: [10, 10], projectiles: [6, 6] });
  ...
  it("matches traced player projectile lifetimes", () => {
    expect([PISTOL_BULLET_LIFETIME, SHOTGUN_BULLET_LIFETIME, MACHINE_GUN_BULLET_LIFETIME, MAGNUM_BULLET_LIFETIME]).toEqual([15, 11, 12, 34].map((frames) => frames / NES_FRAME_RATE));
  ...
  it("advances untraced flank Gunmen through the ROM movement states", () => {
    ...
    expect(gunmanBottomUsesDynamicState(4, 1871, 24)).toBe(true);
    expect(gunmanBottomUsesDynamicState(4, 1871, 128)).toBe(true);
    expect(gunmanFlankEventShotFrames(4, 1871, 24)).toEqual([399]);
    expect(gunmanFlankEventShotFrames(4, 1871, 128)).toEqual([164]);
    expect(gunmanBottomUsesDynamicState(4, 2095, 88)).toBe(true);
    expect(gunmanFlankEventShotFrames(4, 2095, 88)).toEqual([154]);
    expect(gunmanBottomUsesDynamicState(4, 2191, 40)).toBe(true);
    expect(gunmanFlankEventShotFrames(4, 2191, 40)).toEqual([149]);
    ...
    const round4Bottom1871 = createGunmanBottomMovementState(24, 222, 157);
    for (let frame = 49; frame <= 623; frame += 1) {
      const scroll = (1871 + 2 / 3 + frame / 3) * NES_WORLD_Y_SCALE;
      const playerX = frame >= 507 ? 152 : 168;
      const playerY = frame >= 504 && frame < 507 ? 216 : 215;
      advanceGunmanFlankMovement(round4Bottom1871, frame, playerX, playerY, (x, y) => roundActorCollisionAtNes(4, scroll, x, y));
    }
    expect(round4Bottom1871).toMatchObject({ frame: 623, mode: "orbit", heading: 31, timer: 4, x: 153 + 238 / 256, y: 203 + 27 / 256, dead: false });

    const round4Bottom1871Contact = createGunmanBottomMovementState(128, 250, 57);
    for (let frame = 49; frame <= 209; frame += 1) {
      const scroll = (1871 + 2 / 3 + frame / 3) * NES_WORLD_Y_SCALE;
      advanceGunmanFlankMovement(round4Bottom1871Contact, frame, 168, 215, (x, y) => roundActorCollisionAtNes(4, scroll, x, y));
    }
    expect(round4Bottom1871Contact).toMatchObject({ frame: 209, mode: "orbit", heading: 8, timer: 0, x: 159 + 158 / 256, y: 220 + 133 / 256, dead: false });

    const round4Bottom2095 = createGunmanBottomMovementState(88, 181, 48);
    for (let frame = 49; frame <= 210; frame += 1) {
      const scroll = (2095 + 2 / 3 + frame / 3) * NES_WORLD_Y_SCALE;
      const playerX = frame >= 27 ? 120 : 152;
      const playerY = frame >= 24 && frame < 27 ? 216 : 215;
      advanceGunmanFlankMovement(round4Bottom2095, frame, playerX, playerY, (x, y) => roundActorCollisionAtNes(4, scroll, x, y));
    }
    expect(round4Bottom2095).toMatchObject({ frame: 210, mode: "orbit", heading: 8, timer: 1, x: 111 + 229 / 256, y: 221 + 124 / 256, dead: false });

    const round4Bottom2191 = createGunmanBottomMovementState(40, 189, 194);
    for (let frame = 49; frame <= 199; frame += 1) {
      const scroll = (2191 + 2 / 3 + frame / 3) * NES_WORLD_Y_SCALE;
      const playerX = frame >= 27 ? 88 : 104;
      const playerY = frame >= 24 && frame < 27 ? 216 : 215;
      advanceGunmanFlankMovement(round4Bottom2191, frame, playerX, playerY, (x, y) => roundActorCollisionAtNes(4, scroll, x, y));
    }
    expect(round4Bottom2191).toMatchObject({ frame: 199, mode: "orbit", heading: 8, timer: 0, x: 79 + 169 / 256, y: 218 + 14 / 256, dead: false });
