...
const names = "BRK ORA ? SLO IGN ORA ASL SLO PHP ORA ASL ANC IGN ORA ASL SLO BPL ORA ? SLO IGN ORA ASL SLO CLC ORA NOP SLO IGN ORA ASL SLO JSR AND ? RLA BIT AND ROL RLA PLP AND ROL ANC BIT AND ROL RLA BMI AND ? RLA IGN AND ROL RLA SEC AND NOP RLA IGN AND ROL RLA RTI EOR ? SRE IGN EOR LSR SRE PHA EOR LSR ALR JMP EOR LSR SRE BVC EOR ? SRE IGN EOR LSR SRE CLI EOR NOP SRE IGN EOR LSR SRE RTS ADC ? RRA IGN ADC ROR RRA PLA ADC ROR ARR JMP ADC ROR RRA BVS ADC ? RRA IGN ADC ROR RRA SEI ADC NOP RRA IGN ADC ROR RRA SKB STA SKB SAX STY STA STX SAX DEY SKB TXA ANE STY STA STX SAX BCC STA ? SHA STY STA STX SAX TYA STA TXS SHS SHY STA SHX SHA LDY LDA LDX LAX LDY LDA LDX LAX TAY LDA TAX LXA LDY LDA LDX LAX BCS LDA ? LAX LDY LDA LDX LAX CLV LDA TSX LAE LDY LDA LDX LAX CPY CMP SKB DCP CPY CMP DEC DCP INY CMP DEX AXS CPY CMP DEC DCP BNE CMP ? DCP IGN CMP DEC DCP CLD CMP NOP DCP IGN CMP DEC DCP CPX SBC SKB ISC CPX SBC INC ISC INX SBC NOP SBC CPX SBC INC ISC BEQ SBC ? ISC IGN SBC INC ISC SED SBC NOP ISC IGN SBC INC ISC".split(" ");
const modes = "IMP PREIDXIND ? PREIDXIND ZP ZP ZP ZP IMP IMM ACC IMM ABS ABS ABS ABS REL POSTIDXIND ? POSTIDXIND ZPX ZPX ZPX ZPX IMP ABSY IMP ABSY ABSX ABSX ABSX ABSX ABS PREIDXIND ? PREIDXIND ZP ZP ZP ZP IMP IMM ACC IMM ABS ABS ABS ABS REL POSTIDXIND ? POSTIDXIND ZPX ZPX ZPX ZPX IMP ABSY IMP ABSY ABSX ABSX ABSX ABSX IMP PREIDXIND ? PREIDXIND ZP ZP ZP ZP IMP IMM ACC IMM ABS ABS ABS ABS REL POSTIDXIND ? POSTIDXIND ZPX ZPX ZPX ZPX IMP ABSY IMP ABSY ABSX ABSX ABSX ABSX IMP PREIDXIND ? PREIDXIND ZP ZP ZP ZP IMP IMM ACC IMM IND ABS ABS ABS REL POSTIDXIND ? POSTIDXIND ZPX ZPX ZPX ZPX IMP ABSY IMP ABSY ABSX ABSX ABSX ABSX IMM PREIDXIND IMM PREIDXIND ZP ZP ZP ZP IMP IMM IMP IMM ABS ABS ABS ABS REL POSTIDXIND ? POSTIDXIND ZPX ZPX ZPY ZPY IMP ABSY IMP ABSY ABSX ABSX ABSY ABSY IMM PREIDXIND IMM PREIDXIND ZP ZP ZP ZP IMP IMM IMP IMM ABS ABS ABS ABS REL POSTIDXIND ? POSTIDXIND ZPX ZPX ZPY ZPY IMP ABSY IMP ABSY ABSX ABSX ABSY ABSY IMM PREIDXIND IMM PREIDXIND ZP ZP ZP ZP IMP IMM IMP IMM ABS ABS ABS ABS REL POSTIDXIND ? POSTIDXIND ZPX ZPX ZPX ZPX IMP ABSY IMP ABSY ABSX ABSX ABSX ABSX IMM PREIDXIND IMM PREIDXIND ZP ZP ZP ZP IMP IMM IMP IMM ABS ABS ABS ABS REL POSTIDXIND ? POSTIDXIND ZPX ZPX ZPX ZPX IMP ABSY IMP ABSY ABSX ABSX ABSX ABSX".split(" ");
const sizes = "";
const opcodes = new Map(names.flatMap((mnemonic, opcode) => mnemonic === "?" ? [] : [[opcode, { mnemonic, mode: modes[opcode], size: Number(sizes[opcode]) }]]));
